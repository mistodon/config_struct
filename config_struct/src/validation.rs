...
pub enum StructGenerationError {
    #[fail(
        display = "config contains a heterogenous array - all array elements must have the same fields"
    )]
    HeterogenousArray,

    #[fail(
        display = "config contains an invalid key: \"{}\" - all keys must be valid Rust identifiers",
        key_name
    )]
    InvalidFieldName { key_name: String },

    #[fail(
        display = "config struct has an invalid name: \"{}\" - must be a valid Rust identifier",
        name
    )]
    InvalidStructName { name: String },

    #[fail(
        display = "config const has an invalid name: \"{}\" - must be a valid Rust identifier", name
    )]
    InvalidConstName { name: String },
}

pub fn validate_options(options: &Options) -> Result<(), StructGenerationError> {
    if !valid_identifier(&options.struct_name) {
        return Err(StructGenerationError::InvalidStructName {
            name: options.struct_name.clone(),
        });
    }
    if let Some(ref const_name) = options.const_name {
        if !valid_identifier(const_name) {
            return Err(StructGenerationError::InvalidConstName {
...
fn validate_field_name(field_name: &str) -> Result<(), StructGenerationError> {
    if valid_identifier(field_name) {
        ...
        Err(StructGenerationError::InvalidFieldName {
            key_name: field_name.to_owned(),
        })
    }
}
...
mod tests {
    ...
    fn heterogenous_arrays() {
        ...
        assert!(
            validate_array_element_types(&[
                RawValue::Array(vec![RawValue::Unit]),
                RawValue::Array(vec![RawValue::I64(0)]),
            ]).is_err()
        );
    }
}
